...
from theory_exact import compute_theory  #both scripts produce the same result, one is exact, the other exploits autograd to compute gradients and minimize the action
#from theory_numerical_min import compute_theory #both scripts produce the same result, one is exact, the other exploits autograd to compute gradients and minimize the action
import sys


start_time = utils.time.time()
argFile = sys.argv[1] if len(sys.argv) > 1 else 'defaultArgs.json'
args = utils.loadJason(argFile)
if args.onlyTheory: 
...
labels = utils.literal_eval(args.labels)
...
if args.task == 'mnist': 
	teacher_class = teachers.mnist_dataset(args.N, labels, 0)
elif args.task == 'cifar10': 
	teacher_class = teachers.cifar_dataset(args.N, labels, 0 )
elif args.task == 'emnist': 
	teacher_class = teachers.emnist_dataset(args.N,labels, "ABEL-CHJS")#"ABFL-CHIS")
